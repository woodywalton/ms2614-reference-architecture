...
| 8 | **Appendix B Category Coverage Status** | Data table | One row per category: the rule tag (`cat`), label, status, distinct rules that alerted, and last alert time, sorted by tag — the primary evidence panel. Row click offers three evidence views: the rules that alerted (Discover on `m_26_14-metrics-alert-coverage-latest`), the alerts in Security → Alerts filtered on the tag over the last 30 days, and the alert volume per day (`m_26_14-metrics-alert-coverage-by-day`) |
