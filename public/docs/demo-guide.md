...
Five gap tiles line the top row: **No Hardware Inventory (5)**, **Managed, No Software Inventory (0)**, **Unmanaged, No Software Inventory (5)**, **Unknown Encryption Status (5)**, and **No MDM Enrollment (4)** — devices missing a hardware inventory record, a software inventory, a confirmed disk-encryption state, or MDM enrollment. Click any tile to open a Discover view showing exactly which devices — names, OS versions, last-seen times. This is what you hand the ISO instead of a manual audit spreadsheet.

The software-inventory gap is split on `asset.managed` on purpose. The managed tile is the agency's to fix, and the **Managed Assets with No Software Inventory** table at the bottom of the dashboard lists exactly those assets. The unmanaged tile counts unclaimed devices (here the five `UNKNOWN-00x` hosts that the network discovery found but nobody has claimed); those belong to the unknown-device triage runbook, not to a software-inventory push, so they are counted but kept out of the table. On this cluster every uncovered asset is unmanaged, which is why the managed tile and the table both read zero while the unmanaged tile reads five.
...
**How an auditor validates "covered".** A category is covered when a detection rule tagged `Appendix-B-<letter>` alerted in the last 30 days, and the evidence is one click away. Every row of the category table (here, on the Alert Coverage dashboard and on the Readiness Attestation dashboard) opens three views: **the rules that alerted** (Discover on `m_26_14-metrics-alert-coverage-latest`, one row per rule with its last alert time and severity), **the alerts themselves** (Security → Alerts, filtered on the category tag over the last 30 days, each alert linking to its rule), and **the alert volume per day** (Discover on `m_26_14-metrics-alert-coverage-by-day`). Both stores are written by pack transforms from `.alerts-security.alerts-*`, so the table, the Security app and the transforms are three views of the same alert documents, not three sources.

> **How this works:** If you need to document a gap formally, the `m_26_14-poam-drafting-agent` in Elastic Agent Builder can query this coverage data directly using the `m_26_14-readiness-posture-query` tool (one of four pack ES|QL tools, with `m_26_14-asset-inventory-search`, `m_26_14-classification-audit-query` and `m_26_14-retirement-audit-query`) and draft a Plan of Action & Milestones document. It reads the same data as this dashboard — no export needed. The same agent runs unattended on a weekly schedule through the `m_26_14-poam-draft` workflow, which writes the full markdown draft with its finding counts by risk rating to `m_26_14-poam-drafts`; the **POA&M Drafts** table on the Readiness Attestation dashboard lists every draft, and a row click opens it in Discover, so a pre-generated POA&M is on hand for an auditor without a live agent session. The `m_26_14-threat-investigation-agent` can triage specific alerts, and `m_26_14-aar-agent` generates after-action reports from alert history. Every agent conversation is itself logged: the `m_26_14-ai-audit` transform copies each Agent Builder trace into a governed audit store, which is what the LRA asks of AI used inside the logging platform (see section 5).
...
Open [Agent Builder](https://pubsec-m2614-63e0e0.kb.us-east4.gcp.elastic-cloud.com/app/agent_builder/agents) and start a session with `m_26_14-poam-drafting-agent`. It can query the current compliance posture using ES|QL and draft a Plan of Action & Milestones document for any gap you've identified — no data export needed. For a pre-generated draft, open the **POA&M Drafts** table at the bottom of the [Readiness Attestation](https://pubsec-m2614-63e0e0.kb.us-east4.gcp.elastic-cloud.com/app/dashboards#/view/m_26_14-readiness-attestation-dash?_g=(time:(from:now-30d,to:now))) dashboard: the `m_26_14-poam-draft` workflow runs the same agent weekly (it ships disabled; enable it or run it manually from Workflows) and stores each draft with its finding counts, and a row click opens the markdown in Discover.
